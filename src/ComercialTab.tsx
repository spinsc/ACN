...
export default function ComercialTab({ currentUser }) {
  ...
  // Deep-link vindo do painel de Menções ("OP X" clicável) — abre o modal
  // "Ver" da OP em vez de só cair na aba Comercial genérica.
  useEffect(() => {
    const tentarAbrir = () => {
  ...
  const oplsFiltrados = opls.filter(o => {
    ...
    if (filtroBusca && !combinaBusca(o, filtroBusca)) return false;
    return true;
  });
        ...
        {showForm && (
          <div className="sec-body">
            ...
            <div className="form-row">
              ...
              <div className="form-group" style={{maxWidth:140}}><label className="acn-label">MO Adaptação (R$)</label><input type="number" min={0} step="0.01" className="acn-input" style={{width:'100%'}} placeholder="0,00" value={formData.valor_mao_de_obra||''} onChange={e=>setFormData({...formData,valor_mao_de_obra:e.target.value})} /></div>
              <div className="form-group" style={{maxWidth:140}}><label className="acn-label">MO Serralheria (R$)</label><input type="number" min={0} step="0.01" className="acn-input" style={{width:'100%'}} placeholder="0,00" value={formData.valor_mao_de_obra_serralheria||''} onChange={e=>setFormData({...formData,valor_mao_de_obra_serralheria:e.target.value})} /></div>
