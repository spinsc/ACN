...
export default function SerralheriaTab({ currentUser }) {
  ...
  const fetchData = async () => {
    ...
    try {
      if (tab === 'vinculadas') {
        ...
        let query = supabase.from('serralheria_especiais').select('*').order('data_solicitacao', { ascending: false });
        if (filterStatus) query = query.eq('status_solicitacao', filterStatus);
        const { data } = await query;
  ...
  const calcularKPIs = async (tabAtiva) => {
    try {
      ...
      if (tabAtiva === 'vinculadas') {
        const { data: d } = await supabase.from('pcp_pedidos_serralheria').select('*');
        data = d || [];
      } else {
        const { data: d } = await supabase.from('serralheria_especiais').select('*');
        data = d || [];
      }
  ...
  const salvarPedidoEspecial = async () => {
    ...
    try {
      const { error } = await supabase.from('serralheria_especiais').insert([
        {
          numero_pedido: `SER-${Date.now()}`,
          tipo_pedido: formData.tipoPedido,
          descricao_trabalho: formData.descricao,
          quantidade: formData.quantidade,
          peso_kg: formData.peso || null,
      ...
      if (error) {
        ...
        alert('✅ Pedido criado!');
        setFormData({ tipoPedido: 'Projeto Especial', descricao: '', quantidade: 1, peso: 0, observacoes: '' });
        setShowForm(false);
        fetchData();
      }
    } catch (err) {
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.header}>
        <h2 style={styles.title}>⚙️ Serralheria e Estrutura</h2>
                        ...
                        {pedido.status_solicitacao !== 'Concluído' && (
                          <>
                            {pedido.status_solicitacao === 'Pendente' && (
                              <button onClick={() => updateStatus(pedido.id, 'pcp_pedidos_serralheria', 'Em Andamento')} style={{ ...styles.buttonSmall, backgroundColor: '#3b82f6', marginRight: '5px' }}>
                                ▶️
                              </button>
                            )}
                            {pedido.status_solicitacao === 'Em Andamento' && (
                              <button onClick={() => updateStatus(pedido.id, 'pcp_pedidos_serralheria', 'Concluído')} style={{ ...styles.buttonSmall, backgroundColor: '#22c55e' }}>
                                ✅
                              </button>
                            )}
                          </>
          ...
          {showForm && (
            <div style={styles.formCard}>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                ...
                <button onClick={() => { setShowForm(false); setFormData({ tipoPedido: 'Projeto Especial', descricao: '', quantidade: 1, peso: 0, observacoes: '' }); }} style={{ ...styles.buttonSmall, backgroundColor: '#9ca3af', flex: 1 }}>
                  ✖️ Cancelar
                </button>
              </div>
            </div>
                    ...
                    <tr key={pedido.id} style={styles.tableRow}>
                      ...
                      <td style={styles.tableCell}>
                        {pedido.status_solicitacao !== 'Concluído' && (
                          <>
                            {pedido.status_solicitacao === 'Pendente' && (
                              <button onClick={() => updateStatus(pedido.id, 'serralheria_especiais', 'Em Andamento')} style={{ ...styles.buttonSmall, backgroundColor: '#3b82f6', marginRight: '5px' }}>
                                ▶️
                              </button>
                            )}
                            {pedido.status_solicitacao === 'Em Andamento' && (
                              <button onClick={() => updateStatus(pedido.id, 'serralheria_especiais', 'Concluído')} style={{ ...styles.buttonSmall, backgroundColor: '#22c55e' }}>
                                ✅
                              </button>
                            )}
                          </>
