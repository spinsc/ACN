          ...
          <div className="modal-box" style={{maxWidth:520,maxHeight:'90vh',overflowY:'auto'}}>
            ...
            <div style={{marginBottom:10,padding:'8px 10px',border:'1px solid #e2e8f0',borderRadius:6,background:'#fafafa'}}>
              ...
              <div style={{marginTop:8,paddingTop:8,borderTop:'1px dashed #e2e8f0'}}>
                <div style={{fontWeight:700,fontSize:9,color:'#0891b2',marginBottom:5}}>📊 CRM — Acesso a Relatórios</div>
                {[
                  { id:'relatorio_vendedores', label:'👤 Relatório por Vendedor' },
                  { id:'totais_vendas',        label:'💰 Totais de Vendas' },
                  { id:'painel_faturamentos',  label:'📋 Painel de Faturamentos' },
                ].map(({ id, label }) => (
                  <label key={id} style={{display:'flex',alignItems:'center',gap:6,fontSize:10,cursor:'pointer',marginBottom:3}}>
                    <input type="checkbox"
                      checked={(editForm.permissoes_crm||[]).includes(id)}
                      onChange={e => setEditForm(f => ({
                        ...f,
                        permissoes_crm: e.target.checked
                          ? [...(f.permissoes_crm||[]), id]
                          : (f.permissoes_crm||[]).filter(p => p !== id)
                      }))}
...
function PainelAlcadasAprovacao() {
  ...
  const salvar = async () => {
    ...
    if (editando) {
      await supabase.from('compras_alcadas_aprovacao').update(payload).eq('id', editando.id);
    } else {
      await supabase.from('compras_alcadas_aprovacao').insert([{ ...payload, ativo: true }]);
    }
  ...
  const toggleAtivo = async (a) => {
    await supabase.from('compras_alcadas_aprovacao').update({ ativo: !a.ativo }).eq('id', a.id);
    load();
  };
  ...
  return (
    <div className="sec-card">
      <div className="sec-header" style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        <span>✅ Alçadas de Aprovação de Compras</span>
      ...
      <div className="sec-body">
        <p style={{ fontSize:10, color:'#64748b', marginBottom:12 }}>
          Compras com valor igual ou acima do "valor mínimo" de um nível precisam da aprovação de alguém
          com um dos perfis selecionados antes de virar "Comprado". Níveis são aprovados em ordem (do
          menor pro maior). Sem nenhum nível ativo aqui, o fluxo de Compras funciona como hoje (sem aprovação).
        </p>
...
function PainelNfcCfg() {
  ...
  const NFC_KEYS = [
    ...
    { chave:'nfc_base_url',        label:'URL Base da Página NFC (Tag)', tipo:'url',
      desc:'URL onde veiculo.html está hospedado (sem ?chassi=)' },
    { chave:'nfc_portal_url',      label:'URL Base do Portal do Cliente', tipo:'url',
      desc:'URL onde portal.html está hospedado (sem ?token=). Enviada aos clientes como link de acesso.' },
